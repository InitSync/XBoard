...
import net.xboard.plugin.utils.Utils;
			...
			switch (args[0]) {
				...
				case "toggle":
					if (scoreboardHandler.getScoreboard(player.getUniqueId()).isDeleted()) {
						...
						Utils.sendTitle(
							 player,
							 PlaceholderUtils.parse(player, configurationHandler.text("", "messages.yml", "messages.scoreboard-title-on", true)),
							 PlaceholderUtils.parse(player, configurationHandler.text("", "messages.yml", "messages.scoreboard-subtitle-on", true)),
