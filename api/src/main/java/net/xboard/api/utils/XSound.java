...
public enum XSound {
	...
	XSound(String... legacies) {
		...
		if (bukkitSound == null) {
			for (String legacy : legacies) {
				if ((bukkitSound = Data.BUKKIT_NAMES.get(legacy)) != null) break;
			}
		}
		...
		for (String legacy : legacies) Data.NAMES.putIfAbsent(legacy, this);
	}
	...
	public static XSound matchXSound(Sound sound) {
		Preconditions.checkNotNull(sound, "Cannot match XSound of a null sound");
		
		return Data.NAMES.getOrDefault(sound.name(), null);
	}
	...
	public static CompletableFuture<Record> play(Player player, String sound) {
		Preconditions.checkNotNull(player, "Cannot play sound to null player");
	 ...
	 * @since 3.0.0
	 */
	public static Record play(Location location, String sound) {
		Preconditions.checkNotNull(location, "Cannot play sound to null location");
	...
	private static List<String> split(String str, @SuppressWarnings("SameParameterValue") char separatorChar) {
		...
		if (match || lastMatch) list.add(str.substring(start, len));
		return list;
	}
	...
	public static void stopMusic(Player player) {
		Preconditions.checkNotNull(player, "Cannot stop playing musics from null player");
		...
		for (XSound music : musics) {
			if (music.parseSound() != null) player.stopSound(music.parseSound());
		}
	}
	 ...
	 * @since 2.0.0
	 */
	public static BukkitTask playAscendingNote(
		 Plugin plugin,
		 Player player,
		 Entity playTo,
		 ...
		 int ascendLevel,
		 int delay) {
		Preconditions.checkNotNull(player, "Cannot play note from null player");
		Preconditions.checkNotNull(playTo, "Cannot play note to null entity");
	...
	public BukkitTask playRepeatedly(Plugin plugin, Entity entity, float volume, float pitch, int repeat, int delay) {
		Preconditions.checkNotNull(plugin, "Cannot play repeating sound from null plugin");
		Preconditions.checkNotNull(entity, "Cannot play repeating sound at null location");
	...
	public void stopSound(Player player) {
		Preconditions.checkNotNull(player, "Cannot stop playing sound from null player");
	...
	public void play(Entity entity, float volume, float pitch) {
		Preconditions.checkNotNull(entity, "Cannot play sound to a null entity");
	...
	public static class Record {
		...
		public Record(XSound sound) {
			this(sound, DEFAULT_VOLUME, DEFAULT_PITCH);
		}
		
		public Record(XSound sound, float volume, float pitch) {
		...
		public void play(Location updatedLocation) {
			Preconditions.checkNotNull(updatedLocation, "Cannot play sound at null location");
			
			Sound bukkitSound = sound.parseSound();
			assert bukkitSound != null;
			
			if (playAtLocation || player == null) location.getWorld().playSound(updatedLocation, bukkitSound, volume, pitch);
			else player.playSound(updatedLocation, bukkitSound, volume, pitch);
		}
